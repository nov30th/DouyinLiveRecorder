...
kuaishou_cookies_cache = {}  # for kuaishou cookie cache
...
def get_kuaishou_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    if 'kuaishou_updated' not in kuaishou_cookies_cache \
        ...
        # # request to https://id.kuaishou.com/pass/kuaishou/login/passToken with content type application/json;charset=UTF-8 and data  sid=kuaishou.live.web&channelType=UNKNOWN&encryptHeaders=
        # # get the response json and update the cookies
        # pass_token_url = 'https://id.kuaishou.com/pass/kuaishou/login/passToken'
        # pass_token_data = {"sid": "kuaishou.live.web", "channelType": "UNKNOWN", "encryptHeaders": ""}
        # headers_token = headers.copy() | {'content-type': 'application/json'}
        # token_response, response_cookie = get_req_with_headers(url=pass_token_url, headers=headers_token,
        #                                                        json_data=pass_token_data)
        # # checks the json response is not null and result is 1
        # if token_response:
        #     token_response_json = json.loads(token_response)
        #     if token_response_json.get('result') == 1:
        #         # using the response json to update the cookies, split response_cookie with \n then finds the Set-Cookie and update the cookies
        #         if token_response and response_cookie:
        ...
        #     else:
        #         print(f"Failed to update cookies for Kuaishou. {token_response}")
        # else:
        #     print(f"Failed to update cookies for Kuaishou. No response.")
    ...
    if not play_list_item:
        print(f"No stream data for {url}")
